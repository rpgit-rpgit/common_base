...
main()
{
	...
	pbh = pa_blk_init(4, 5);
	CHECK_RET_PTR(pa_blk_init, pbh);

	node = pa_getnode(pbh);
	CHECK_RET_PTR(pa_getnode, node);

	ret = pa_relnode(pbh, node);
	CHECK_RET_INT(pa_relnode, ret);
	...
	free(pbh);
	printf("\nfreed once %x", (unsigned)pbh);
	scanf("%d", &ret);
	free(pbh);
	printf("\nfreed twice %x", (unsigned)pbh);
/*	
	ret = pa_blk_dest(pbh);	
	CHECK_RET_INT(pa_blk_dest, ret);
	ret = pa_blk_dest(pbh);	
	CHECK_RET_INT(pa_blk_dest, ret);
	return(0);
*/
